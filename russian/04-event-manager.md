...
\imgcapt{img/EventSource-01.png}{Окно «Источники событий»}
...
\imgcapt{img/EventSource-04-01.png}{Окно свойств источника событий по TCP/IP, вкладка «Приемник событий»}
...
\imgcapt{img/EventHandler-05-03.png}{Окно «Настройка обработчика событий «Контроль цепочки событий» для контроля связи с передатчиком ТР-100 GSM III»}
...
\imgcapt{img/Eventhandler-08-01.png}{Перезакрытие объектов, окно «Параметры обработчика (Параметры передачи)», вкладка «Параметры сообщений»}
...
Обработчик событий «Ретранслятор в Sur-Gard» позволяет организовать передачу информации о принятых событиях во внешнее программное обеспечение (ПО). События передаются в текстовом формате SurGard.

В качестве канала передачи информации можно использовать любой канал, поддерживающий протокол TCP/IP. 

#### Параметры группы

Настройки группы обработчиков событий «Ретранслятор в Sur-Gard» на вкладке «Общие» полностью совпадают с общими настройками групп обработчиков событий, которые детально рассмотрены выше.

##### Вкладка «Канал»

На вкладке «Канал» указываются параметры организации соединения с внешним ПО. 

Так как в качестве канала связи обработчик «Ретранслятор в Sur-Gard» использует сеть, реализующую протокол TCP/IP, то для установки соединения между обработчиком и внешним ПО один из них должен выступать в роли сервера, а второй - в роли клиента.

Роль, в которой будет выступать обработчик при организации соединения, задается параметром «Режим организации соединения».

\imgcapt{img/Eventhandler-09-01.png}{Окно «Параметры группы (Ретранслятор в Sur-Gard)», вкладка «Канал», режим «Сервер»}

Если обработчик событий выступает в роли сервера и в компьютер установлено несколько сетевых адаптеров, либо один адаптер использует несколько IP-адресов, то с помощью параметра «Адрес интерфейса» можно указать IP-адрес, на котором обработчик событий должен ожидать входящее подключение. Параметр «Номер порта» предназначен для того, чтобы указать порт, к которому будет ожидаться подключение.

\imgcapt{img/Eventhandler-09-02.png}{Окно «Параметры группы (Ретранслятор в Sur-Gard)», вкладка «Канал», режим «Клиент»}

Если обработчик событий выступает в роли клиента, то для него необходимо указать адрес и порт сервера, к которому необходимо выполнять подключение.

> Необходимо отметить, что для обработчика событий «Ретранслятор в Sur-Gard», выступающего в роли клиента, можно указать несколько адресов сервера: в том случае, если не удается установить связь с первым адресом в списке, обработчик сделает попытку подключиться к следующему и так далее.

Для добавления сервера в список, необходимо указать адрес и порт сервера в качестве значений для параметров «Адрес сервера» и «Порт», после чего нажать на кнопку «Добавить».

Для удаления сервера, необходимо выбрать его в списке серверов и нажать на кнопку «Удалить».

##### Тесты и контроль связи

Для того, чтобы контролировать наличие соединения при отсутствии информации на передачу, обработчик «Ретранслятор в Sur-Gard» формирует тестовые посылки и контролирует их прием.

При потере соединения создается системное событие с кодом «ZZYB». При восстановлении соединения по каналу связи обработчик событий «Ретранслятор в Sur-Gard» создает системное событие с кодом «ZZYC». Если значение параметра «Тайм-аут восстановления связи» не равно нулю, то при продолжительном отсутствии соединения системные события кодом «ZZYB» будут создаваться с периодом, заданным значением этого параметра.

##### Вкладка «Параметры»

На вкладке «Параметры» задаются параметры предназначенные для управления передачей информаци по каналу связи.

\imgcapt{img/Eventhandler-09-03.png}{Окно «Параметры группы (Ретранслятор в Sur-Gard)», вкладка «Параметры»}

Значение параметра «Интервал подтверждения доставки» зависит от пропускной способности канала связи используемого обработчиком «Ретранслятор в Sur-Gard» и производительности компьютеров, на которых запущен модуль «Менеджер событий». Например, если в качестве канала связи используется GPRS, то во избежание лавинообразного увеличения объема информации в очереди на передачу рекомендуется увеличивать значение параметра «Интервал подтверждения доставки» до 90 секунд.

Значение параметра «Интервал отправки HeartBeat» задает время формирования тестовых посылок.

При отсутствии соединения обработчик «Ретранслятор в Sur-Gard» накапливает информацию в очереди на передачу, а после восстановления связи осуществляет передачу информации накопленной в очереди. 
Чтобы управлять объемом и актуальностью данных, которые накапливаются в очереди на передачу, существует параметр «Интервал хранения не переданных данных». Если период хранения данных в очереди на передачу больше, чем значение данного параметра, то такие данные будут автоматически удалены из очереди на передачу. Кроме того, если пропускная способность канала связи ухудшилась и в очереди накопились данные, передать которые не представляется возможным, нажатием на кнопку «Очистить очередь на передачу» можно принудительно удалить всю информацию, накопленную в очереди на передачу в настоящий момент.

Если поставлена отметка напротив параметра «Включать дату и время в события», то передача будет осуществляться в расширенном формате SurGard, включающем время события.

Если поставлена отметка напротив параметра «Протоколировать обмен», то на жесткий диск компьютера будет сохраняться протокол обмена обработчика событий по сети TCP/IP. Эта информация бывает нужной при выяснении причин возникновения проблем при установке соединения или доставке информации по каналу связи. Не рекомендуется включать протоколирование обмена самостоятельно, без запроса из службы технической поддержки ООО НТКФ «Си-Норд».

#### Параметры обработчика

Настройки обработчика событий «Ретранслятор в Sur-Gard» во многом совпадают с рассмотренными выше общими настройками обработчиков событий, за исключением вкладки «Классы событий».

##### Вкладка «Классы событий»

На вкладке «Классы событий» можно выбрать классы событий, которые будут передаваться обработчиком.

\imgcapt{img/Eventhandler-09-04.png}{Окно «Параметры обработчика (Параметры передачи)», вкладка «Классы событий»}
...
\imgcapt{img/Eventhandler-07-01.png}{Окно «Параметры обработчика (Ретранслятор в «Облако»)», вкладка «Общие»}
...
\imgcapt{img/Eventhandler-07-02.png}{Окно «Параметры обработчика (Ретранслятор в «Облако»)», вкладка «Канал»}
...
### Ретранслятор в ПАК ВсМК (Росгвардия)

Обработчик событий «Ретранслятор в ПАК ВсМК (Росгвардия)» предназначен для ретрансляции информации о событиях в ПАК ВсМК Росгвардии. Для работы обработчика необходимы учётные данные для доступа к ПАК ВсМК, настроенные идентификаторы объектов на вкладке «Росгвардия» в Менеджере объектов, а также запущенная "Служба интеграции с ПАК ВсМК" (см. далее).

#### Параметры группы

Настройки группы обработчиков событий «Ретранслятор в ПАК ВсМК (Росгвардия)» на вкладке «Общие» полностью совпадают с общими настройками групп обработчиков событий, которые детально рассмотрены выше.

##### Вкладка «Параметры»

\imgcapt{img/Eventhandler-10-01.png}{Параметры группы обработчика «Ретранслятор в ПАК ВсМК (Росгвардия)»}

Параметр «Логин ПАК ВсМК» задает имя пользователя для авторизации в ПАК ВсМК.

Параметр «Пароль ПАК ВсМК» задает пароль для авторизации в ПАК ВсМК.

Значение параметра «Интервал подтверждения доставки» задает время ожидания подтверждения доставки сообщения в ПАК ВсМК. Если за указанный интервал подтверждение получено не будет, то обработчик выполнит отправку не подтвержденной информации еще раз.

При отсутствии соединения обработчик «Ретранслятор в ПАК ВсМК (Росгвардия)» накапливает информацию в очереди на передачу, а после восстановления связи осуществляет передачу информации накопленной в очереди. Чтобы управлять объемом и актуальностью данных, которые накапливаются в очереди на передачу, существует параметр «Интервал хранения непереданных данных». Если период хранения данных в очереди на передачу больше, чем значение данного параметра, то такие данные будут автоматически удалены из очереди на передачу.

Если поставлена отметка напротив параметра «Включить протоколирование обмена», то на жесткий диск компьютера будет сохраняться протокол обмена обработчика событий. Эта информация бывает нужной при выяснении причин возникновения проблем при установке соединения или доставке информации. Не рекомендуется включать протоколирование обмена самостоятельно, без запроса из службы технической поддержки ООО НТКФ «Си-Норд».

Если поставлена отметка напротив параметра «Включить расширенное протоколирование», то на жесткий диск компьютера будет сохраняться детальный протокол обмена, который может быть полезен при диагностике проблем.

Кроме того, если в очереди накопились данные, передать которые не представляется возможным, нажатием на кнопку «Очистить очередь на передачу» можно принудительно удалить всю информацию, накопленную в очереди на передачу в настоящий момент.

#### Параметры обработчика

Настройки обработчика событий «Ретранслятор в ПАК ВсМК (Росгвардия)» полностью совпадают с рассмотренными выше общими настройками обработчиков событий.

#### Служба интеграции с ПАК ВсМК

Для обеспечения двусторонней интеграции «Центра охраны» с ПАК ВсМК используется служба интеграции RosguardIntegrationGateway. Служба автоматически устанавливается при установке «Центра охраны» и работает как служба Windows.

Служба выполняет следующие функции:

* автоматизированная передача вызовов на реагирование (тревог) и тревожных событий в ПАК ВсМК;
* получение статусов обработки вызовов (тревог) и действий групп задержания из ПАК ВсМК;
* отображение действий групп задержания в журнале действий «Центра охраны».

Для корректной работы «Центра охраны» со службой интеграции необходимо, чтобы в Менеджере событий была создана и настроена группа обработчиков «Ретранслятор в ПАК ВсМК (Росгвардия)», а для объектов на вкладке «Росгвардия» в Менеджере объектов были указаны идентификаторы объектов из ПАК ВсМК.

##### Настройка конфигурации службы

Конфигурация службы интеграции хранится в каталоге установки службы и включает следующие файлы:

* `appsettings.yaml` — основные параметры службы (подключение к ПАК ВсМК, приём уведомлений, логирование);
* `event_codes.yaml` — таблица соответствия кодов событий «Центра охраны» кодам типов событий ПАК ВсМК.

Основные параметры конфигурации в файле `appsettings.yaml`:

* **rosguard.api_endpoint** — адрес API ПАК ВсМК. Значение по умолчанию может быть необходимо заменить на актуальный адрес, предоставленный оператором ПАК ВсМК;
* **webhook.enabled** — включение приёма уведомлений от ПАК ВсМК (по умолчанию включено);
* **webhook.listen_address** — порт для приёма уведомлений от ПАК ВсМК (по умолчанию `:27003`);
* **webhook.allowed_ips** — список IP-адресов, с которых разрешён приём уведомлений от ПАК ВсМК. Актуальные адреса можно посмотреть в разделе «Настройка API» веб-интерфейса ПАК ВсМК.

Документация по API ПАК ВсМК доступна по адресу <https://pakvcmk.ru/docs/monitoring.html>.

##### Настройка сети

Для полноценной работы службы интеграции необходимо выполнить следующие действия по настройке сети на компьютере с «Центром охраны»:

1. **Открыть порт для приёма уведомлений.** Порт, указанный в параметре `webhook.listen_address` (по умолчанию 27003), должен быть открыт для входящих подключений из внешней сети. Этот порт используется ПАК ВсМК для отправки уведомлений о действиях групп задержания.

2. **Убедиться в доступности API ПАК ВсМК.** Адрес, указанный в параметре `rosguard.api_endpoint`, должен быть доступен с компьютера, на котором установлена служба интеграции.

3. **Настроить список разрешённых IP-адресов.** В параметре `webhook.allowed_ips` необходимо указать IP-адреса серверов ПАК ВсМК, с которых будут поступать уведомления. Уведомления с адресов, не входящих в этот список, будут отклонены. Актуальные IP-адреса и настройка адреса для приёма уведомлений доступны в разделе «Администрирование» → «Настройка API» веб-интерфейса ПАК ВсМК. Настройка приёма уведомлений осуществляется через аккаунт с правами администратора мониторинговой компании.

\imgcapt{img/webhook_setup.png}{Настройка API и адресов для приёма уведомлений в веб-интерфейсе ПАК ВсМК}

##### Перезапуск службы

При изменении конфигурации службы необходимо перезапустить службу интеграции. Для этого откройте оснастку «Службы» Windows (services.msc), найдите службу «RosguardIntegrationGateway» и выполните её перезапуск.

#### Монитор тревог

Для наблюдения за состоянием интеграции с ПАК ВсМК и просмотра активных тревог предназначено приложение «Монитор тревог» (RosguardIntegrationGatewayUI.exe), которое поставляется вместе с «Центром охраны». После запуска, приложение размещается в области уведомлений Windows и отображает текущее состояние подключения к ПАК ВсМК.

##### Состояние подключения

Значок приложения в области уведомлений отображает текущее состояние подключения:

* **«Росгвардия: подключена»** — служба интеграции работает, подключение к ПАК ВсМК установлено.

\imgcapt{img/gui_monitor_svc_available_and_connected.png}{Служба интеграции работает, подключение к ПАК ВсМК установлено}

* **«Росгвардия: нет подключения»** — служба интеграции работает, но подключение к ПАК ВсМК отсутствует. Необходимо проверить сетевые настройки и доступность API ПАК ВсМК.

\imgcapt{img/gui_monitor_svc_avail_rg_not_connected.png}{Служба интеграции работает, подключение к ПАК ВсМК отсутствует}

* **«Служба недоступна»** — служба интеграции не запущена или недоступна. Необходимо проверить состояние службы в оснастке «Службы» Windows.

\imgcapt{img/gui_monitor_svc_not_available.png}{Служба интеграции недоступна}

##### Окно монитора тревог

При щелчке по значку приложения в области уведомлений открывается окно монитора тревог с таблицей активных тревог, переданных в ПАК ВсМК.

\imgcapt{img/gui_monitor_window.png}{Окно «Монитор тревог»}

В строке состояния окна отображается статус подключения к ПАК ВсМК и время последнего обновления данных.

Для завершения работы приложения «Монитор тревог» щёлкните правой кнопкой мыши по значку в области уведомлений и выберите пункт «Выход». Завершение работы монитора не влияет на работу службы интеграции.

#### При миграции «Центра охраны»

При переносе «Центра охраны» на другой компьютер необходимо учесть следующие особенности, связанные с интеграцией с ПАК ВсМК:

* если файл конфигурации `appsettings.yaml` был изменён после установки, перенести его в каталог службы на новом компьютере;
* если файл таблицы соответствия кодов событий `event_codes.yaml` был изменён после установки, также перенести его;
* открыть порт для приёма уведомлений от ПАК ВсМК (по умолчанию 27003) на новом компьютере;
* при необходимости обновить параметр `webhook.allowed_ips` в конфигурации службы;
* при изменении адреса компьютера повторно настроить адреса доставки уведомлений в разделе «Администрирование» → «Настройка API» веб-интерфейса ПАК ВсМК через аккаунт с правами администратора мониторинговой компании.
...
На вкладке «Режим подключения» окна «Связь с Облаком» отображается режим подключения к Облаку, который был выбран при установке «Центра охраны». Выбор режима определяет облачные сервисы, которые будут использоваться, и данные, передаваемые в Облако.

\imgcapt{img/EvMan-07-CloudConnection.png}{Окно «Связь с Облаком», вкладка «Режим подключения»}
...
\imgcapt{img/EvMan-07-CloudConnection-02.png}{Окно «Связь с Облаком», вкладка «Контактные данные»}
...
\imgcapt{img/EvMan-07-CloudConnection-03.png}{Окно «Связь с Облаком», вкладка «UID Центра охраны»}
...
\imgcapt{img/EvMan-08-AboutProgram.png}{Окно «О программе»}
