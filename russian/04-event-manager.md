...
## Источники событий (Службы)

Основное назначение «Источников событий (служб)» - бесперебойная работа источников независимо от «Менеджера событий». Если «Менеджер событий» будет выключен, то события которые приходят на источники будут сохраняться в памяти до восстановления связи с «Менеджером событий» или до перезапуска службы

\imgcapt{img/EventSource-15-01.png}{Службы «Источники событий»}

Доступ к настройкам источников событий можно получить, если выбрать пункт «Источники событий (Службы)...» в меню модуля, которое появляется при щелчке правой кнопкой мыши на иконке модуля в системной области панели задач.

Для того, чтобы получить доступ к «Конфигуратору служб» и сохранить изменения, произведенные в нем, пользователь должен обладать разрешением «Изменять настройки» для модуля «Менеджер событий».

Нажав на кнопку меню «Добавить» можно выбрать нужную службу/источник событий из списка установленных в системе.

\imgcapt{img/EventSource-15-02.png}{Меню «Конфигуратора Источники событий»}

### Универсальный источник событий (Служба)

Источник событий с возможностью изменения протокола обмена и транспортного протокола.

Помимо стандартных настроек, содержит выбор Канала подключения (TCP/IP или COM порт) и Протокол источника событий (на данный момент доступен только протокол "LLR8000").

\imgcapt{img/EventSource-15-03.png}{Изменение настроек «Универсального источника событий»}
