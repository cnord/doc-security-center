...
Для отображения объекта на ситуационной карте оператору "Центра охраны", обладающему правом "Размещать объект на карте", следует задать в модуле "Менеджер объектов" географические координаты объекта, разместив его на карте местности.

Для этого без перехода в режим редактирования объекта необходимо нажать кнопку "Разместить на карте...", расположенную рядом с полем "Адрес" на вкладке "Объект". Окно "Карта", вызванное данным действием, отображает карту сервиса Яндекс.Карты. На ней маркером отмечается местоположение объекта в соответствии со значением поля "Город", указанном при регистрации в "Облаке", и поля "Адрес", указанном на вкладке "Объект". Кроме того, в окне приводится краткая информация об объекте, а именно: номер, название и адрес объекта, а также комментарий для ГБР, введенный на вкладке "Комментарий".

Наведя на него курсор и удерживая нажатой левую кнопку мыши, маркер можно переместить для указания более точного местоположения объекта на карте. Увеличение или уменьшение карты достигается с помощью ползунка изменения масштаба, а передвижение по карте возможно с помощью курсора. Выпадающий список позволяет изменить выбранный по умолчанию тип карты "Схема" на "Спутник", "Гибрид", "Народная карта", "Народная + спутник".

![Вкладка «Объект»: размещение объекта на карте][id-05-04-01]

После того, как маркер будет установлен, следует нажать кнопку "Сохранить" для сохранения созданных изменений или "Отменить" для отмены сохранения. Полученные координаты объекта будут сохранены в "Облаке" и в базе данных "Центра охраны". 
...
Для удаления сохраненных координат объекта с карты необходимо кликнуть по кнопке "Удалить с карты" и затем подтвердить удаление. При этом информация о координатах объекта будет удалена как из Облака, так и из базы данных "Центра охраны".
...
В модуле "Менеджер объектов" оператор, обладающий правом "Редактировать изображения объекта", может загрузить с жесткого диска в Облако или удалить из Облака изображения выбранного объекта. Приложение "Тревога в ГБР" отображает хранящиеся в Облаке изображения объекта, с которого поступил сигнал тревоги, для предоставления сотрудникам охранных предприятий информации о виде объекта и наиболее удобных подъездах к нему. 
...
С помощью кнопки "Выбрать" в адресной строке "Файлы" можно выбрать для загрузки одно или несколько изображений объекта, информационный объем которых не превышает 5 МБ, в форматах PNG и JPEG. Для того чтобы быстрее найти их на жестком диске и правильно сохранить в Облаке, лучше поместить изображения объекта в папке, имя которой соответствует номеру объекта, либо присвоить файлам с изображениями имена, начинающиеся с номера объекта. 
