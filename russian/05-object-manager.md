...
Для настройки оборудования следует выделить нужный объект и перейти в режим редактирования. Из представленного на вкладке «Оборудование» списка систем необходимо выбрать используемый на объекте тип оборудования: «ТР-100 GSM III / Союз GSM», «AlarmView», «Lonta-202», «RS200», «Puper type 5» или «Neman». В случае, если на объекте установлено оборудование другой системы, нужно указать тип «Другое». 

Для типов оборудования «ТР-100 GSM III / Союз GSM», «Lonta-202», «Puper type 5», «Neman» и «Другое» следует указать значения параметров для настройки оборудования. Важно помнить, что при копировании объекта для нового объекта копируется только информация об установленном типе оборудования, но не значения заданных параметров.
...
* «Норд GSM», «Норд GSM WRL», «Норд GSM Mini» или «Норд GSM Air»
...
Вкладка «Росгвардия» предназначена для настройки параметров объекта, необходимых для интеграции с ПАК ВсМК Росгвардии. Для передачи событий в ПАК ВсМК должна быть создана и настроена группа обработчиков «Ретранслятор в ПАК ВсМК (Росгвардия)» в Менеджере событий.

\imgcapt{img/ObjMngr-25-RosGvardia.png}{Вкладка «Росгвардия»}

На вкладке «Росгвардия» можно указать следующие параметры:

* «Особо важный объект» — чекбокс, отмечается для особо важных объектов;
* «Тип объекта» — выпадающий список с типом объекта;
* «Номер телефона» — контактный номер телефона объекта;
* «Идентификатор объекта» — UUID объекта из ПАК ВсМК (обязательное поле для корректной работы ретранслятора). Получение идентификатора описано ниже;
* «Комментарий к адресу» — дополнительная информация об адресе;
* «Характеристика объекта» — описание характеристик объекта.

### Получение идентификатора объекта из ПАК ВсМК

Для корректной ретрансляции событий необходимо указать идентификатор объекта, зарегистрированного в ПАК ВсМК. Ниже описана процедура получения этого идентификатора.

1. Войдите в веб-интерфейс ПАК ВсМК.

\imgcapt{img/PakVsMK-01-ObjectList.png}{Список объектов в веб-интерфейсе ПАК ВсМК}

2. Создайте новый объект и заполните обязательные поля (подразделение, договор ИТО, пультовый номер, тип, категория, наименование, адрес).

\imgcapt{img/PakVsMK-02-ObjectCard.png}{Карточка объекта в ПАК ВсМК}

3. Сохраните объект и нажмите кнопку «Отправить на согласование».

\imgcapt{img/PakVsMK-03-SendForApproval.png}{Отправка объекта на согласование}

4. Дождитесь смены статуса объекта на «на проверке».

\imgcapt{img/PakVsMK-04-StatusChecking.png}{Объект в статусе «на проверке»}

5. После согласования объект переходит в статус «на охране».

\imgcapt{img/PakVsMK-05-StatusOnGuard.png}{Объект в статусе «на охране»}

6. Откройте карточку объекта и скопируйте UUID из адресной строки браузера.

\imgcapt{img/PakVsMK-06-ObjectID.png}{Идентификатор объекта в адресной строке браузера}

7. Вставьте скопированный UUID в поле «Идентификатор объекта» на вкладке «Росгвардия» в Менеджере объектов Центра охраны.

\imgcapt{img/ObjMngr-25-RosGvardia.png}{Поле «Идентификатор объекта» на вкладке «Росгвардия»}
