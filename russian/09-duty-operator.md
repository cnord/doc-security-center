...
Кнопка «Отработка...» предназначена для открытия окна «Отработка тревоги». Кнопка доступна только в том случае, если текущим является тревожный объект.
...
Ссылка «Видео на объекте...» позволяет оператору просмотреть видео, передаваемое видеокамерами, установленными на объекте. При переходе по ссылке открывается окно браузера, отображающее видео-поток с камер.
