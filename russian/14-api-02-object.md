...
Максимальное количество элементов, которое необходимо вернуть в результате выполнения запроса. Если значение для параметра не указано, то метод вернет количество элементов, не превышающее значение параметра [`DefaultPageSize`](#api-settings-default-page-size), указанное в настройках службы «C.Nord HTTP-API Service». Примечание: параметр [`DefaultPageSize`](#api-settings-default-page-size) не может превышать быть больше 200.
