...
Элемент JSON, содержащий все поля объекта:
...
Метод предназначен для получения списка объектов. В качестве критериz для поиска объекта могжет использоваться номер договора.
