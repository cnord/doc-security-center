...
PartNumber & number & Номер раздела (почти всегда совпадает с номером, запрограммированным в контрольную панель, установленную на объекте) \\ \arrayrulecolor{light-gray}\hline
...
В теле запроса должен быть передан элемент JSON с [полями раздела](#api-part-json), который нужно создать. При создании раздела обязательно указать только название. Значения для всех остальных полей, включая номер раздела, можно не указывать: будет использовано значение по умолчанию.

Если указан идентификатор раздела, то он будет проигнорирован.

Если номер раздела не указан, то новому разделу будет присвоен свободный номер по формуле max(PartNumber) + 1.
