...
PartNumber & number & Номер раздела (обязательный, почти всегда совпадает с номером, запрограммированным в контрольную панель, установленную на объекте, натуральное число) \\ \arrayrulecolor{light-gray}\hline
...
В теле запроса должен быть передан элемент JSON с [полями раздела](#api-part-json), который нужно создать. При создании раздела обязательно указать название, номер раздела и описание. Значения для всех остальных полей можно не указывать: будет использовано значение по умолчанию.
